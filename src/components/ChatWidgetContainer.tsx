...
const DEFAULT_IFRAME_URL = 'https://chat-widget.antbuddy.com';
...
class ChatWidgetContainer extends React.Component<Props, State> {
  ...
  constructor(props: Props) {
    ...
    if (!token) {
      throw new Error('A `token` is required to run the AntBuddy chat!');
    } else if (!isValidUuid(token)) {
      console.error(
        `The \`token\` must be a valid UUID. (Received invalid \`token\`: ${token})`
      );
      console.error(
        `If you're missing a AntBuddy \`token\`, you can get one by signing up for a free account at https://antbuddy.com/signup`
      );
  ...
  async componentDidMount() {
    ...
    // whether to display the AntBuddy branding or not in the chat window
  ...
  customEventHandlers = (event: any) => {
    ...
    switch (type) {
      ...
      case 'antbuddy:toggle':
        return this.handleToggleOpen();
      case 'antbuddy:customer:set':
  ...
  handleChatLoaded = () => {
    ...
    this.send('antbuddy:plan', {plan: subscriptionPlan});
  };
  ...
  emitToggleEvent = (isOpen: boolean) => {
    this.send('antbuddy:toggle', {isOpen});
